...
const PokemonMovesList = (props) => {
    let data = props.pokemonMovesData
    let name = data.names.filter((res)=>{
        return res.language.name === "en"
    })
    return(
        <TouchableOpacity style={styles.containerTouchable} onPress={props.onMovePressed}>
            ...
            <View style={{flex: 0.3, flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center'}}>
                {/* <Image style={{width: 40, height: 40}} source={require('../assets/icons/ic-fire-types.png')}/> */}
                {showMovesTypeIcon(data)}
            </View>
        </TouchableOpacity>
    );
}
