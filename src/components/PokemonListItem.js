...
const PokemonListItem = (props) => {
    ...
    let pokemonTypes = props.pokemonTypes

    return(
        <TouchableOpacity style={styles.containerTouchable} onPress={props.onPokemonListPressed}>
            <View style={{flex: 0.2}}>
                {/* <Image style={{width: 60,height: 60}} source={{uri: getPokemonImage(pokemonId)}}/> */}
                <Image style={{width: 60,height: 60}} source={{uri: props.pokemonSprites.front_default}}/>
            </View>
            <View style={{flex: 0.5, flexDirection: 'column'}}>
                <Text style={{fontSize: 18}}>{uppercaseLetter(props.pokemonName)}</Text>
                <Text style={{fontSize: 16, color: '#989898', marginTop: 5}}>{'#'+pokemonId}</Text>
            </View>
            <View style={{flex: 0.3, flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center'}}>
                    {pokemonTypes.map((values)=>{
                        let imgSource;
